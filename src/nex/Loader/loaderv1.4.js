...
;(function($){
	...
	Loader.extend({
		...
		getDefaults : function(opt){
			var _opt = {
				...
				_lmt : 0,
				debug : false,
				exportsName : '_export',//做导出用 最好是没创建一个Loader exportsName都应该设置不同 因为在IE下会有onload 通知不及时问题
				exportsApi : 'exports',//全局导出api 最好用这个因为支持IE
				includeApi : 'include',//全局加载api
	...
	Loader.fn.extend({
		_init : function(opt) {
			...
			Loader.__loader = self;
			
			window[opt.exportsName] = {};
			window[opt.includeApi] = function(){
				self.include.apply( self,arguments );	
			};
			window[opt.exportsApi] = function(){
		...
		_sysEvents : function(){
			...
			self.bind("onLoadScriptComplete._require",function(name,opt){
				/*如果不开启loadOrder情况下由于如果多个脚本同时加载完 IE会先把脚本执行后在触发onload导致触发不及时*/
				/*
				*正常的表达式
				*  if( opt.loadOrder ) { 
				...
				*  }
				*/
				if( !(Nex.isIE && !opt.loadOrder) ) {//opt.loadOrder
					Loader.exports[ name ] = window[opt.exportsName];//如果loadOrder:false 那么这句在IE下不应该执行 IE下需要通过exports() 或者 include(..)等函数辅助才能正确拿到导出的数据
				}
				self.resetExports();
			},self);
			...
			self.bind("onLoadScriptComplete._require",function(name,opt){
				...
				$.each( self.__loaderCallBack.concat([]),function(i,data){
					...
					$.each( scripts,function(i,d){				 
						if( (d in cache) &&  !self._isLoadScript( cache[d] ) ) {// //$.type(cache[d]) !== 'boolean'
							complete = false;
							return false;
						}					 
					} );
		...
		*/
		_load : function(){
			...
			opt.path2name[ name ] = opt.currScriptAliasName;
			
			var cache = false;
			//检查加载缓存
			//opt._loadCache[opt.currScriptAliasName] !== null 
			if( self._isLoadScript(opt._loadCache[opt.currScriptAliasName])                 
		...
		*/
		load : function( list,fn ){
			var self = this,
				...
				opt = this.configs;
			//记录当前执行的模块
			var currExports = /*opt.loadOrder ? opt.currScriptAliasName : */self.getInteractiveName();
			//console.log( list, currExports,'%');
			if( list !== undef ) {	
			...
			if( $.isFunction(fn) ) {
				...
				Nex.array_insert( 0,[{
										...
										callBack : function(){
											...
											//console.log( currExports,window[opt.exportsName],fn )
											if( currExports ) {
			...
			if( opt._list.length ) {
				...
				if( opt.loadOrder && currExports ) {// && currExports
		...
		include : function(){
			...
			if( argvs.length === 1 ) {
				var v = argvs[0];
				if( $.isFunction( v ) ) {
					if( /*Nex.isIE  && */!opt.loadOrder ) {//这步应该是在不开启loadOrder 且是IE浏览器情况下使用
						var currExports = self.getInteractiveName();
						window[opt.exportsName] = {};
						Loader.exports[ currExports ] = window[opt.exportsName];	
					}
					var s = v(window[opt.exportsName]);	
					if( s !== undef ) {
						self.exports( s );	
					}
				} else {
				...
				if( argvs[0] === '' || ( $.isArray( argvs[0] && !argvs[0].length ) ) ) {
					self.include( argvs[1] );
					return;
				} else {
					if( !$.isArray( argvs[0] ) ) {
						argvs[0] = [ argvs[0] ];	
