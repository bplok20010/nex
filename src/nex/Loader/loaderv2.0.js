		...
		_getLoader : function(){
			...
			if( !self.__loader ) {
				self.__loader = Nex.Create('Nex.Loader',{
					autoLoad : false,
					loadOrder : false,
					cjsRequireRegExp : /[^.]\s*[require|include|Nex\.require|Nex\.include]\s*\(\s*["']([^'"\s]+)["']\s*\)/g,
					exportsName : '_export',
					exportsApi : 'exports',
					includeApi : 'require include define',
	...
	Loader.extend({
		...
		getDefaults : function(opt){
			var _opt = {
				...
				checkExtReg : /(\.css|\.js|\.php|\.jsp|\.py|\.txt|\.htm|\.html|\.php4|\.php5|\.php3)$/,
	...
	Loader.fn.extend({
		...
		*/
		"load" : function( list,fn,errback ){//deps,callback,errback
			...
			if( $.isFunction(fn) ) {
				var _deps = self.getInlineDeps( fn );
			...
			if( nlist.length ) {
				...
				isShim = true;
				var fn = undef,
					eventType = opt.strictDeps ? 'onLoadScriptSuccess' : 'onLoadScriptComplete',
					errback = undef;
				...
				eid = self.bind( eventType+'._load',function( m ){
					...
					if( complete ) {
						self._addDeps( _list );
						self.load( _list,_callback,_errback );
			...
			if( $.isFunction(fn) ) {
				...
				window[opt.exportsName+'CallBack'] = {
					...
					callBack : function(){
						...
						//如果模块存在 执行回调 并且回调里面的exports保持到当前模块中
						if( this.moduleName ) {
							Nex.Loader.exports[ this.moduleName ] = $.isPlainObject( window[opt.exportsName] ) && $.isEmptyObject( window[opt.exportsName] ) ? undef : window[opt.exportsName];
						}
						//清空exports
						self.resetExports();
					}
