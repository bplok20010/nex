	...
	return {
		...
		*/
		...
		extend : function(subclass,superClass,overrides){
			...
			if( superClass && Nex.getClass( superClass ) ) {//Nex.classes // (superClass in Nex)
...
Nex._Manager.fn.extend({
		...
		for( var id in comps ) {
			...
			if( obj ) {
				...
				for( var i=0,len=name.length;i<len;i++ ) {
					var n = name[i];
					if( gname.length && gname.indexOf(n)!== -1 ) {// name === gname
						c.push( obj );
						break;
					}	
				}
	...
	*/
	...
	create : function(){
		...
		if( len > 1 ) {
			var xtype = argvs[0];
			if( Nex.isNexConstructor( xtype ) && $.isFunction( xtype ) ) {
